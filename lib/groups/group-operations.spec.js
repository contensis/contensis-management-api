...
describe('Group Operations', () => {
    ...
    describe('Create group', () => {
        ...
        it('for valid group', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let group = await client.groups.create(specs_utils_spec_1.defaultGroups[0]);
            expect(global.fetch).toHaveBeenCalledTimes(2);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups`,
                specs_utils_spec_1.getDefaultRequest('POST', null, JSON.stringify(specs_utils_spec_1.defaultGroups[0]))
            ]);
            expect(group).not.toBeNull();
            expect(group.id).toEqual(specs_utils_spec_1.defaultGroups[0].id);
        });
    ...
    describe('Update group', () => {
        ...
        it('for valid group', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let group = await client.groups.update(specs_utils_spec_1.defaultGroups[0]);
            expect(global.fetch).toHaveBeenCalledTimes(2);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].id}`,
                specs_utils_spec_1.getDefaultRequest('PUT', null, JSON.stringify(specs_utils_spec_1.defaultGroups[0]))
            ]);
            expect(group).not.toBeNull();
            expect(group.id).toEqual(specs_utils_spec_1.defaultGroups[0].id);
        });
    ...
    describe('Delete group', () => {
        ...
        it('for valid group', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let result = await client.groups.delete(specs_utils_spec_1.defaultGroups[0].id);
            expect(global.fetch).toHaveBeenCalledTimes(2);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].id}`,
    ...
    describe('Add user to group', () => {
        ...
        it('for valid group and user', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let result = await client.groups.addUser(specs_utils_spec_1.defaultGroups[0].id, specs_utils_spec_1.defaultUsers[0].id);
            expect(global.fetch).toHaveBeenCalledTimes(2);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].id}/users/${specs_utils_spec_1.defaultUsers[0].id}`,
                specs_utils_spec_1.getDefaultRequest('PUT')
            ]);
            expect(result).toEqual(null);
        });
    });
    describe('Remove user from group', () => {
        ...
        it('for valid group and user', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let result = await client.groups.removeUser(specs_utils_spec_1.defaultGroups[0].id, specs_utils_spec_1.defaultUsers[0].id);
            expect(global.fetch).toHaveBeenCalledTimes(2);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].id}/users/${specs_utils_spec_1.defaultUsers[0].id}`,
    ...
    describe('Has user in group', () => {
        describe('for a positive result', () => {
            ...
            it('and valid group and user', async () => {
                let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
                let result = await client.groups.hasUser(specs_utils_spec_1.defaultGroups[0].id, specs_utils_spec_1.defaultUsers[0].id);
                expect(global.fetch).toHaveBeenCalledTimes(2);
                expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
                expect(global.fetch.calls.mostRecent().args).toEqual([
                    `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].id}/users/${specs_utils_spec_1.defaultUsers[0].id}`,
    ...
    describe('Add child group to group', () => {
        ...
        it('for valid group and child group', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let result = await client.groups.addChildGroup(specs_utils_spec_1.defaultGroups[0].id, specs_utils_spec_1.defaultGroups[1].id);
            expect(global.fetch).toHaveBeenCalledTimes(2);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].id}/childGroups/${specs_utils_spec_1.defaultGroups[1].id}`,
                specs_utils_spec_1.getDefaultRequest('PUT')
            ]);
            expect(result).toEqual(null);
        });
    ...
    describe('Remove child group from group', () => {
        ...
        it('for valid group and child group', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let result = await client.groups.removeChildGroup(specs_utils_spec_1.defaultGroups[0].id, specs_utils_spec_1.defaultGroups[1].id);
            expect(global.fetch).toHaveBeenCalledTimes(2);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].id}/childGroups/${specs_utils_spec_1.defaultGroups[1].id}`,
    ...
    describe('Gets users in group', () => {
        ...
        it('by group id', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let users = await client.groups.getUsersByGroupId(specs_utils_spec_1.defaultGroups[0].id);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
        ...
        it('by group name', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let users = await client.groups.getUsersByGroupName(specs_utils_spec_1.defaultGroups[0].name);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].name}/users`,
                specs_utils_spec_1.getDefaultRequest()
            ]);
            expect(users).not.toBeNull();
            expect(users.items.length).toEqual(2);
    ...
    describe('Gets groups in group', () => {
        ...
        it('by group id', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let groups = await client.groups.getChildGroupsByGroupId(specs_utils_spec_1.defaultGroups[0].id);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].id}/groups`,
                specs_utils_spec_1.getDefaultRequest()
            ]);
            expect(groups).not.toBeNull();
            expect(groups.items.length).toEqual(2);
        ...
        it('by group name', async () => {
            let client = Zengenti.Contensis.Client.create(specs_utils_spec_1.getDefaultConfig());
            let users = await client.groups.getChildGroupsByGroupName(specs_utils_spec_1.defaultGroups[0].name);
            expect(global.fetch.calls.first().args[0]).toEqual(specs_utils_spec_1.getDefaultAuthenticateUrl());
            expect(global.fetch.calls.mostRecent().args).toEqual([
                `http://my-website.com/api/management/security/groups/${specs_utils_spec_1.defaultGroups[0].name}/groups`,
                specs_utils_spec_1.getDefaultRequest()
            ]);
            expect(users).not.toBeNull();
            expect(users.items.length).toEqual(2);
