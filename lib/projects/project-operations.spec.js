...
describe('Project Operations', () => {
    describe('Get project', () => {
        ...
        it('Without root url (relative)', async () => {
            ...
            expect(global.fetch.calls.mostRecent().args).toEqual([
                ...
                specs_utils_spec_1.getDefaultFetchRequest(null, true)
            ]);
            expect(project).not.toBeNull();
            expect(project.name).toEqual('project1');
        });
    ...
    describe('Create project', () => {
        ...
        beforeEach(() => {
            specs_utils_spec_1.setDefaultSpy(global, newProject);
            Zengenti.Contensis.Client.defaultClientConfig = null;
            Zengenti.Contensis.Client.configure({
                fetchFn: global.fetch
            });
        ...
        it('With specified root url', async () => {
            ...
            expect(global.fetch.calls.mostRecent().args).toEqual([
                'http://my-website.com/api/management/projects',
                specs_utils_spec_1.getDefaultFetchRequest('POST', null, JSON.stringify(newProject))
            ]);
            expect(project).not.toBeNull();
