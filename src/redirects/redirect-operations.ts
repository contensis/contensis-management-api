...
let listMappers: { [key: string]: MapperFn } = {
  pageIndex: (value: number, options: PageOptions, params: ClientParams) =>
    (options && options.pageIndex) || params.pageIndex,
  pageSize: (value: number, options: PageOptions, params: ClientParams) =>
    (options && options.pageSize) || params.pageSize,
};

export class RedirectOperations implements IRedirectOperations {
  constructor(
    private httpClient: IHttpClient,
    private contensisClient: ContensisClient
  ) {}

  get(id: string): Promise<Redirect> {
    let url = UrlBuilder.create(
      '/api/management/projects/:projectId/redirects/:id',
      {}
    )
      .addOptions(id, 'id')
  ...
  private ensureRedirectIsValid(
    ...
    if (
      redirect.appendSourcePathToDestination === undefined ||
      typeof redirect.appendSourcePathToDestination !== 'boolean'
    ) {
      throw new Error(
        'A valid appendSourcePathToDestination value needs to be specified.'
      );
    }
